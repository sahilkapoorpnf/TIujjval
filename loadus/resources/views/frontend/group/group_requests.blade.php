        ...
        <div class="row d-flex align-items-stretch">
            ...
            <div class="col">
                <div class="dshContent">
                    <h2 class="title blue">Group Requests</h2>
                    <hr>
                    <ul class="notiMsg">

                        <?php
                        foreach ($group_requests as $req) {
                            ...
                            if ($req->group_owner_id == (Auth::guard('user')->user()->id) && $req->group_owner_id == $req->sent_by) {
                                ...
                                $text = 'You have invited <b>' . $req->first_name . '</b> to join your group <a href="group-details/' . $req->group_id . '"><b>' . $req->group_name . '</b></a>';
                                ...
                                $text = '<b>' . $req->first_name . '</b> has requested you to join your group <a href="group-details/' . $req->group_id . '"><b>' . $req->group_name . '</b></a>';
                                ...
                                $text = 'You have request <b>' . $group_owner_details->first_name . '</b> to join his group <a href="group-details/' . $req->group_id . '"><b>' . $req->group_name . '</b></a>';
                                $html = '<a class="btn acp cancelJoin" href="javascript:void(0)" data-id="' . Crypt::encryptString($req->gfm_Id) . '">Cancel Join</a>';
                            } else if ($req->group_owner_id != (Auth::guard('user')->user()->id) && $req->member_id == (Auth::guard('user')->user()->id) && $req->sent_by != (Auth::guard('user')->user()->id)) {

                                // $group_owner_details = userDetails($req->group_owner_id);

                                $text = '<b>' . $group_owner_details->first_name . '</b> has invited you to join his group <a href="group-details/' . $req->group_id . '"><b>' . $req->group_name . '</b></a>';
...
// return false;
        ...
        $.ajax({
            url: '<?php echo action('GroupController@cancelInvite') ?>',
            type: "post",
            ...
            success: function (data) {
                if (data.status === true) {
                    ...
                    window.location.href = "{{url('group-join-request-list')}}";
                } else {
...
// return false;
        ...
        $.ajax({
            url: '<?php echo action('GroupController@rejectInvite') ?>',
            type: "post",
            data: {"_token": "{{ csrf_token() }}", "gfm_id": gfm_id, "group_id": group_id},
            dataType: 'json',
            beforeSend: function () {
...
// return false;
        ...
        $.ajax({
            url: '<?php echo action('GroupController@rejectJoin') ?>',
            type: "post",
            data: {"_token": "{{ csrf_token() }}", "gfm_id": gfm_id, "group_id": group_id},
            dataType: 'json',
            beforeSend: function () {
...
// return false;
        ...
        $.ajax({
            url: '<?php echo action('GroupController@acceptInvite') ?>',
            type: "post",
            data: {"_token": "{{ csrf_token() }}", "gfm_id": gfm_id, "group_id": group_id},
            dataType: 'json',
            beforeSend: function () {
