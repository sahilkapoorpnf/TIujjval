...
<div class="clearfix cbutton">
    <div class="pull-right">
       <a href="{{url('admin/group')}}"><button class="btn btn-success btn-flat" data-toggle="tooltip" title="" data-original-title="Back">Back</button></a>
    </div> 
    <p>Group ID : {{$data->id}}</p>
    <p>Group Owner Name : {{$data->groupuser->first_name}}</p>
</div>
...
<div class="box box-success color-palette-box">
    ...
    <div class="box-header with-border">
        <h3 class="box-title">Group Name : {{ $data->name }}</h3>        
    </div>
    <div class="box-body">
...
<script>
    ...
    $(document).ready(function () {
        ...
        mytable = $('#example').DataTable({
            ...
            "ordering": false,
            "ajax": {
                "url": "{{url('admin/group/getuserdata')}}",
                "type": "POST",
            ...
            if(Array.isArray(userIds) && userIds.length){
                swal({
                    ...
                    if (willSubmit) {
                        $.ajax({
                            type: 'POST',
                            url: '{{url("admin/group/addgroupmember")}}',
